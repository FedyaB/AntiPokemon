...
class AppIntro
{
  ...
  // METHODS
  public AppIntro(BaseActivity ctx, int language)
  {
  ...
  private void drawAppleLeaf(Canvas canvas, int deltaTimeMs)
  {
    ...
    if ( (m_timeState > 2* TIME_LEAF)  )
    {
      ...
      drawButton(canvas, m_rectBtnStart, m_strStart, cheatMode ? 0x00FF00 : 0x92DCFE, 0x1e80B0, opa);
      if (hasWeb)
      {
  ...
  private RectF[] cheatSequence;
  private int cheatIndex = 0;
  public boolean cheatMode = false;

  private void initCheatSequence() {
    cheatSequence = new RectF[4];
    cheatSequence[0] = new RectF(0, 0, m_scrW / 6, m_scrH / 6);
    cheatSequence[1] = new RectF(0, 0, m_scrW / 6, m_scrH / 6);
    cheatSequence[2] = new RectF(m_scrW * 5 / 6, 0, m_scrW, m_scrH / 6);
    cheatSequence[3] = new RectF(m_scrW * 5 / 6, 0, m_scrW, m_scrH / 6);
  }
  ...
  public boolean	onTouch(int x, int y, int touchType)
  {
    ...
    {
      m_ctx.setView(BaseActivity.ViewMode.APPLICATION);
      return false;
    }
    ...
    if (cheatSequence[cheatIndex].contains(x, y)) {
